...
- 🎵 **v2.9.3 MiniMax Music3 提示词增强自动更新版**：新增纯文本音乐前期策划节点，支持歌词生成、逐字保留、指定段落润色、纯音乐、结构/语言/BPM/拍号/调性/时长与 Caption 规划；默认继续使用贞贞的平价AI小屋 `bytedance/doubao-seed-2.1-pro`，并支持现有全部已启用扩展 LLM 渠道。安装包包含固定的 MiniMax 官方 Music 3 Skill 快照（1,022 文件、18 个风格族索引、1,000 个模板），按渐进披露每次只读取有限参考；来源提交和树哈希可核验。既有 v2.9.2 启动优化、节点、默认值和旧画布均不回退。
