...
- 🎞️ **v2.9.5 FashVSR 与 Seedance 2.5 原生 1080P 自动更新版**：新增独立 `FashVSR 视频超分` 节点，精确调用 `FashVSR_video_upscale`，对一个 480P、3–15 秒视频执行受控上传、同任务查询、本地下载与完整解码；一次真实任务已从 854×480 成功输出 1920×1024。Seedance 2.5 国内/Global 六模型新增官方 `native1080p` 选项，按费用边界只做文档合同核对。MiniMax Music3 不是本版才补入：它已随 v2.9.3 正式发布，并在 v2.9.4 与 v2.9.5 安装包中完整保留官方固定 Skill 快照、1,022 个上游文件、18 个索引和 1,000 个模板。
