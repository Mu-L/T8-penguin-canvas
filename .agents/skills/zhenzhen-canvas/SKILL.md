...
> 2026-08-06 v2.8.1 release candidate: the owner explicitly authorized one fixed-source Electron/NSIS and GitHub automatic-update release before affected-user install, multi-device collaboration, and installed-upgrade evidence is added. The fixed scope is Qwen Image 3.0, MiniMax H3 OW and their saved workflows, MiniMax H3 official-Skill and digital-human MV prompting updates, plus the side-effect-free RunningHub `app-info` two-site fallback; existing defaults and paid-submit replay boundaries remain unchanged. The only allowed evidence deferral token is `owner-approved-post-release-v2.8.1`; deferred checks remain unpassed. Electron/NSIS, private-backend encryption, post-build, provenance, sealed recovery, GitHub assets, immutable Tag, and Latest are pending the single formal release flow and must not be claimed early.
