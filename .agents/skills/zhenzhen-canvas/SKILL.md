...
> 2026-08-05 v2.8.0 release candidate: the owner explicitly authorized the single fixed-source Electron/NSIS and GitHub automatic-update release before deferred live-provider, affected-user VPN/install, multi-device collaboration, and installed-upgrade evidence is added. The fixed scope is RunningHub cross-site WebApp ownership recovery plus Provider generation, Creator stream persistence, seedance.nz upload, and trusted-result download hardening without rolling back any later capability. The only allowed evidence deferral token is `owner-approved-post-release-v2.8.0`; deferred checks remain unpassed. Electron/NSIS, post-build, provenance, sealed recovery, GitHub assets, Tag, and Latest are pending that one release flow and must not be claimed early.

> 2026-08-05 v2.8.0 release-candidate RunningHub cross-site WebApp lookup correction: RunningHub may return HTTP 200 with numeric `901` (`WEBAPP_NOT_EXISTS`) and no retryable prose when a WebApp ID is queried through the wrong site. The shared RH site classifier now recognizes only identity/resource mismatch codes (`423`, `802`, `806`, `807`, `811`, `901`, `1002`, `1004`) and retries the other configured site, while validation, capacity, billing, timeout, 5xx, and generation failures still do not replay. This covers app-info, submit, query, cancel, and upload for the ordinary RunningHub node, wallet node, RH Tools, and the persistent RH Toolbox capability layer. The RH tool editor now persists both `cn` and `intl` sites returned by lookup. A real read-only Provider check confirmed overseas WebApp `2075149522166181890` resolves directly on `.ai` with four fields and no fallback, while domestic WebApp `2084704734417612802` resolves from an `.ai` request by one `901` fallback to `.cn` with six fields. Focused site routing passes 6/6, app-info route/security passes 2/2, and TypeScript passes. No paid submission or task execution/download was performed; release-candidate authorization is not release evidence.
