...
> 2026-08-08 v2.8.4 release preparation: the owner explicitly authorizes publishing before affected-user installation, full provider-quality-matrix, multi-device collaboration, and installed-upgrade evidence is collected. The only accepted missing-manifest deferral is `owner-approved-post-release-v2.8.4`, and deferral must never be reported as passing evidence. Scope is fixed to the standalone `seedance25` node, the Video `Seedance 2.5` tab, six exact domestic/global Standard T2V/I2V/Multi models, six credential-free workflows, and pre-paid-submit MIME/byte/FFprobe duration gates. Existing Video defaults, Seedance 2.0, explicit saved-canvas values, and earlier capabilities must not regress. The fixed source commit, Electron/NSIS artifacts, Tag, Latest state, remote downloads, sizes, and digests remain pending until the single `dist:release` flow completes.
...
The same Seedance 2.5 source passed 100 adjacent tests, TypeScript, public-surface validation, capability synchronization at 31 actions / 225 runtime entries / 76 of 76 nodes with zero public gaps, and the 2,862-module production web build. A real local Canvas UI check created both temporary entries, selected the affordable-house provider and Seedance 2.5 tab, verified all six models plus I2V and 30/10/10/50 Multi guidance, recorded zero console errors, and removed both temporary nodes. Electron packaging, source commit, push, and release are outside this unreleased change.
