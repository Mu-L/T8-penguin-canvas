...
> 2026-08-08 v2.8.5 release authorization: version metadata, README, Release notes, features, the collaboration-evidence deferral token, and automatic-update identity are synchronized to v2.8.5. The release scope is limited to the Hailuo H3 Global / FLUX 3 Video feature below, its eleven workflows, dedicated recovery routing, generated catalogs, and tests. The only permitted formal flow starts from one source commit already pushed to `origin/main`, uses `T8-penguin-canvas-release-v2.8.5` on `codex/release-v2.8.5`, and must pass required local-private sidecars, encrypted backend, both runtime archives, Electron native rebuild, NSIS, post-build, provenance, sealed recovery, full GitHub downloads of all three update artifacts, fixed Tag/target, Latest, and recovery cleanup. No failed gate may be deferred. Affected-user installation, the full Provider quality matrix, multi-device collaboration, and installed-upgrade evidence is explicitly deferred under `owner-approved-post-release-v2.8.5`; deferral is not a pass. Final publication facts and artifact digests may be written only after the real release converges.
