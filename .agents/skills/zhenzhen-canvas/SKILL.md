...
> 2026-08-05 v2.7.9 release candidate: the owner explicitly authorized packaging and publishing before deferred live-provider, affected-user VPN/install, multi-device collaboration, and installed-upgrade evidence is added. The fixed release scope is the protocol-isolated Seedance 2.0 prompt enhancer plus the RunningHub Chromium forbidden-header compatibility correction. The only allowed evidence deferral token is `owner-approved-post-release-v2.7.9`; deferred checks remain unpassed. Electron/NSIS, post-build, provenance, sealed recovery, GitHub automatic-update assets, Tag, and Latest are pending the single fixed-source release flow and must not be claimed early.

> 2026-08-05 development capability: custom RunningHub/RH WebApp lookup no longer hand-writes the upstream `Host` header on app-info, submit, query, cancel, or upload. The URL is the sole upstream authority, and the shared Chromium system-fetch bridge removes browser-managed authority, connection, and body-framing request headers while preserving semantic headers such as Authorization and Content-Type. Node stream/dispatcher uploads remain on Node fetch and retain required Content-Length; the separate DNS-pinned raw media transport retains its socket Host authority. Renderer calls to local `/api/proxy/runninghub/*` remain intentional so credentials stay in the local backend. Focused bridge/routing tests pass 17/17, RH-adjacent tests pass 42/42, system-network/security-adjacent tests pass 74/74, and TypeScript, feature-sync, public-clean, and the production frontend build pass. No affected-user VPN/install reproduction, Electron package, or release evidence exists yet and none may be claimed.

> 2026-08-05 development capability: Canvas node type `seedance20-prompt-enhancer` is implemented as a protocol-isolated Seedance 2.0 prompt enhancer alongside MiniMax H3. It accepts `text/image/video`, emits `text`, supports AUTO/T2V/I2V/FL-I2V/MultiRef/VideoEdit/VideoExtend/TrackFill/Combined, AUTO/4–15 seconds, AUTO/1–20 shots, two reference syntaxes, template fusion, subtitles, stability constraints, and a soft length target. It has no audio input and must never claim attached-audio analysis. The fixed Canvas default remains `seedance-nz` / `bytedance/doubao-seed-2.1-pro`, while the workshop and every enabled LLM extension remain explicitly selectable. Full videos remain temporal evidence; paid generation is single-submit; missing providers or models fail closed; credentials remain global-only. Focused tests are 8/8, combined RunIntent tests are 5/5, node port/Run audits are 15/15, capability graph audits are 10/10, TypeScript, feature-sync, public-clean and the production frontend build pass, and generated capability coverage is 74/74 with zero public gaps. The production build contains a dedicated Seedance 2.0 node chunk. Live-provider, installed Electron, Electron packaging, and release evidence are not yet executed and must not be claimed.
