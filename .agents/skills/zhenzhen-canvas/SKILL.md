...
> 2026-08-07 v2.8.3 release candidate authorized: package, public capability manifest, README, release notes, and the collaboration-evidence deferral gate are synchronized for the fixed-source MV Music Master release. The new `mv-music-master` workbench covers material authorization, 5.000–14.990-second sentence-safe audio segmentation, director settings, bounded visual-bible and per-segment Prompt generation, storyboard/video candidate review and adoption, versioned EDL composition with exactly one complete original-song track, playback-bound QC, and formal delivery receipts. Provider requests bind real material SHA-256, approved provider, canonical model and task type; ambiguous writes are not replayed automatically, STOP propagates through paid work and downloads, and stale upstream changes revoke downstream delivery. Defaults remain `seedance-nz` / `bytedance/doubao-seed-2.1-pro`. Electron/NSIS assets, Tag, Latest, remote re-download, and final hashes remain unclaimed until the one authorized `dist:release` flow succeeds. Affected-user installs, the full live-provider quality matrix, multi-device collaboration, and installed-upgrade evidence are explicitly deferred under `owner-approved-post-release-v2.8.3` and must not be reported as passed.
