...
> 2026-08-06 v2.8.2 release authorization: Electron 33 main-process `session.fetch` no longer crashes when a Provider or file service echoes a Chinese Windows username, userData/install path, or filename in a response header. Chromium may expose that UTF-8 header as a Unicode JS string, but Electron copies it into Undici `Headers`, whose ByteString conversion rejects characters such as `自` (33258). The shared system-network bridge now applies a reversible UTF-8-byte/latin1 representation only to Unicode response-header values for main-process requests with no WebContents; renderer requests, ASCII/latin1 headers, URLs, filesystem paths, bodies, Chromium system proxy/PAC/TUN/VPN/IPv4/IPv6 routing, and paid-write replay boundaries remain unchanged. A real Electron control without the bridge reproduces the reported `Headers.set`/`ClientRequest` stack, while the bridged process returns the same Chinese `Content-Disposition` path and body successfully. Packaged Chinese userData plus a Chinese input directory and image filename also pass the Seedream I2I local-read/convert/save path. Focused regression passes 23/23. The owner authorized one fixed-source Electron/NSIS release with exact post-release evidence deferral token `owner-approved-post-release-v2.8.2`; artifact hashes, Tag, Latest and installed-user evidence remain pending and must not be claimed before the formal flow completes.
...
- The next authorized fixed-source release is `v2.8.2`, limited to the Windows Unicode response-header compatibility correction. It must use the single `dist:release` flow and exact evidence deferral token `owner-approved-post-release-v2.8.2`; do not predeclare artifact or GitHub verification facts.
- The current released desktop and automatic-update baseline is `v2.8.1`: <https://github.com/T8mars/T8-penguin-canvas/releases/tag/v2.8.1>. It is published, non-prerelease, and GitHub Latest.
- Its fixed `v2.8.1` release Tag and Release target point to packaged-source commit `6e6be689df35edc14d407697ed56b8312aec22a4`. Later metadata-only commits on `main` must not move that Tag.
...
- The authorized v2.8.1 flow produced exactly one actual Electron/NSIS artifact set. Installer `T8-PenguinCanvas-Setup-2.8.1.exe` is 1,317,461,216 bytes with SHA-256 `ca5285af6da80c849ed73ffcf8e760ae3b4e44263a3af02c30a81d19c68e8e59`; its blockmap is 1,373,047 bytes with SHA-256 `f87cd204c8d4d1726b7477d99d99d65f922615f72563c9c477d2815daba6f1b0`; `latest.yml` is 362 bytes with SHA-256 `4d50ebbdaca9b9ede21db42c37631dc763480a54aa09fdf8e81556d38d43fa80`. Post-build, provenance, sealed recovery, GitHub digest/size metadata, full prepublish and independent postpublish remote downloads, final Latest verification, and recovery cleanup passed.
