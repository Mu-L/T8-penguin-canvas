...
> 2026-08-12 v2.9.0 release authorization: version metadata, README, Release notes, features, the collaboration-evidence deferral token, and automatic-update identity are synchronized to v2.9.0. The scope is limited to `zhenzhen-image-gk-v2`, the three Wan Image models, the eight Qwen3-TTS/MiniMax/Mureka audio models, twelve credential-free workflows, and their Provider/proxy/complete ordered-output/capability integration; no existing default channel, default model, saved field, Provider protocol, node, workflow, or prior release capability may be rolled back. The only permitted formal flow starts from one source commit already pushed to `origin/main`, uses `T8-penguin-canvas-release-v2.9.0` on `codex/release-v2.9.0`, and must pass local-private sidecars, encrypted backend, both runtime archives, Electron native rebuild, NSIS, post-build, provenance, sealed recovery, exact draft ownership, full GitHub downloads of all three automatic-update assets, fixed Tag/target, Latest, and recovery cleanup. No failed technical gate may be deferred. Affected-user installation, multi-device collaboration, and installed-upgrade evidence are explicitly deferred under `owner-approved-post-release-v2.9.0`; deferral is not a pass. Final publication facts and artifact digests may be written only after the real release converges.

> 2026-08-12 affordable-house image/audio development capability: the current `https://api.seedance.nz/docs/llms.txt` snapshot (Last-Modified `Tue, 11 Aug 2026 21:03:05 GMT`, SHA-256 `9088f0a28ff140b2bc754b7938c8bb8d7155ace5a861b24ffc8d40fb0a7ea84c`) and reference `ComfyUI_Seedance@38041c0492` add Grok `zhenzhen-image-gk-v2`, Wan Image `wan-2.7-global-t2i` / `wan-2.7-global-i2i` / `wan-2.7-global-i2i-pro`, Qwen3 TTS, MiniMax Music/Speech/Voice Clone, and Mureka V8/V9 BGM. Mureka `n=1-3` outputs are preserved and materialized in original order without sorting, deduplication, or truncation. Twelve credential-free workflows are saved, while existing credentials, default channel, default model, historical nodes, and old canvas data remain unchanged. Focused plus adjacent regression passes 84/84 together with TypeScript, Node syntax, the 2,863-module production build, public/RH/JSON gates, and capability synchronization at 31 actions / 247 runtime entries / 77 of 77 nodes / zero public gaps. All twelve models were submitted exactly once in live verification and reached succeeded; thirteen media outputs were downloaded and decoded, including two ordered Mureka V9 results. The sanitized report is `output/latest-image-audio-live-20260812/report.json`. Credentials were process-only, and source, workflows, settings, and report contain no API key, task ID, signed URL, or raw response. Electron packaging and release remain pending the authorized fixed-source flow above.
